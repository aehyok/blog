import{_ as s,o as n,c as a,N as l}from"./app.0f646e78.js";import"./chunks/theme.33468903.js";const g=JSON.parse('{"title":"","description":"","frontmatter":{"theme":"vuepress","highlight":"tomorrow-night"},"headers":[{"level":2,"title":"安装nginx","slug":"安装nginx","link":"#安装nginx","children":[]},{"level":2,"title":"设置nginx服务自动启动","slug":"设置nginx服务自动启动","link":"#设置nginx服务自动启动","children":[]},{"level":2,"title":"在/etc/nginx/conf.d文件夹下新建一个html.conf","slug":"在-etc-nginx-conf-d文件夹下新建一个html-conf","link":"#在-etc-nginx-conf-d文件夹下新建一个html-conf","children":[]},{"level":2,"title":"创建项目","slug":"创建项目","link":"#创建项目","children":[]},{"level":2,"title":"通过脚本执行打包后，自动拷贝到linux服务器","slug":"通过脚本执行打包后-自动拷贝到linux服务器","link":"#通过脚本执行打包后-自动拷贝到linux服务器","children":[]},{"level":2,"title":"","slug":"","link":"#","children":[]}],"relativePath":"javascript/2022-08-17-nginx.md","lastUpdated":1678577188000}'),e={name:"javascript/2022-08-17-nginx.md"},p=l(`<p>携手创作，共同成长！这是我参与「掘金日新计划 · 8 月更文挑战」的第23天，<a href="https://juejin.cn/post/7123120819437322247" title="https://juejin.cn/post/7123120819437322247" target="_blank" rel="noreferrer">点击查看活动详情</a>。</p><blockquote><p>前言：大家好，我是 <code>那个曾经的少年回来了</code>。10年前我也曾经年轻过，如今已步入被淘汰的年龄，但如下幡然醒悟，所以活在当下，每天努力一点点，来看看2024年的时候自己会是什么样子吧，2024年的前端又会是什么样子，而2024年的中国乃至全球又会变成什么样子，如果你也有想法，那还不赶紧行动起来。期待是美好的，但是更重要的是要为美好而为之奋斗付诸于行动。</p></blockquote><p>opensuse 通过zypper可以方便的进行软件管理，类似centos的yum 软件。</p><h2 id="安装nginx" tabindex="-1">安装nginx <a class="header-anchor" href="#安装nginx" aria-hidden="true">#</a></h2><div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki material-theme-palenight" tabindex="0"><code><span class="line"><span style="color:#A6ACCD;">// 查找nginx</span></span>
...
<span class="line"><span style="color:#A6ACCD;"></span></span></code></pre></div><h2 id="设置nginx服务自动启动" tabindex="-1">设置nginx服务自动启动 <a class="header-anchor" href="#设置nginx服务自动启动" aria-hidden="true">#</a></h2><div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki material-theme-palenight" tabindex="0"><code><span class="line"><span style="color:#A6ACCD;">// 设置nginx开机自动启动</span></span>
...
<span class="line"><span style="color:#A6ACCD;"></span></span></code></pre></div><h2 id="在-etc-nginx-conf-d文件夹下新建一个html-conf" tabindex="-1">在/etc/nginx/conf.d文件夹下新建一个html.conf <a class="header-anchor" href="#在-etc-nginx-conf-d文件夹下新建一个html-conf" aria-hidden="true">#</a></h2><div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki material-theme-palenight" tabindex="0"><code><span class="line"><span style="color:#A6ACCD;">server {</span></span>
...
<span class="line"><span style="color:#A6ACCD;">    }</span></span>
<span class="line"><span style="color:#A6ACCD;">}</span></span>
<span class="line"><span style="color:#A6ACCD;"></span></span></code></pre></div><h2 id="创建项目" tabindex="-1">创建项目 <a class="header-anchor" href="#创建项目" aria-hidden="true">#</a></h2><p>这里我直接使用我自己的项目，这个项目也在github上开源了，谁都可以看到 <a href="https://github.com/aehyok/blog" target="_blank" rel="noreferrer">https://github.com/aehyok/blog</a></p><p>当然这里面已经包含了我的半自动化脚本和全自动 github Actions yml脚本，推送代码到指定分支的时候就会自动部署。</p><h2 id="通过脚本执行打包后-自动拷贝到linux服务器" tabindex="-1">通过脚本执行打包后，自动拷贝到linux服务器 <a class="header-anchor" href="#通过脚本执行打包后-自动拷贝到linux服务器" aria-hidden="true">#</a></h2><p>在package.json中引入zx</p><div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki material-theme-palenight" tabindex="0"><code><span class="line"><span style="color:#A6ACCD;">npm i zx</span></span>
<span class="line"><span style="color:#A6ACCD;"></span></span></code></pre></div><p>然后在package.json中的scripts</p><div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki material-theme-palenight" tabindex="0"><code><span class="line"><span style="color:#A6ACCD;">&quot;buildtome&quot;: &quot;npm run build &amp;&amp; zx ./scripts/prebuild.mjs&quot;,</span></span>
<span class="line"><span style="color:#A6ACCD;"></span></span></code></pre></div><p>prebuild.js文件内容如下</p><div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki material-theme-palenight" tabindex="0"><code><span class="line"><span style="color:#A6ACCD;">import { $, argv } from &quot;zx&quot;;</span></span>
<span class="line"><span style="color:#A6ACCD;"></span></span>
...
<span class="line"><span style="color:#A6ACCD;">}</span></span>
<span class="line"><span style="color:#A6ACCD;"></span></span></code></pre></div><p>先来判断是在window执行的脚本，还是在mac下（linux应该也可以不过我没尝试过）两者获取路径的方式是不太一样。 然后其实就是通过scp指令，将本地打包编译好的目录，拷贝到服务器对应的目录上</p><h2 id="" tabindex="-1"><a class="header-anchor" href="#" aria-hidden="true">#</a></h2><ul><li></li></ul>`,22),o=[p];function t(c,i,r,C,A,d){return n(),a("div",null,o)}const u=s(e,[["render",t]]);export{g as __pageData,u as default};
